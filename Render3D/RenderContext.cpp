...
void RenderContext::project(Vector3 & threeD)
{
	if (threeD.z > 0.0f)
	{
		...
		threeD.y = ((-threeD.y / threeD.z) * m_State.m_D) + m_State.m_Center.y;
...
void RenderContext::unProject(Vector3 &twoD)
{
	...
	twoD.y = (twoD.z * -(twoD.y - m_State.m_Center.y)) / m_State.m_D;
}
	...
	float fov, float front, float back)
{
	...
	m_State.m_Fov = Clamp(fov, 0.0f, PI);
	m_State.m_Front = front;
	m_State.m_Back = back;

	float fAspect = 1.0f;
	float fH = 2 * sin(m_State.m_Fov / 2.0f);
	float fW = (1.0f / fAspect) * fH;
	float fF = 2 * cos(m_State.m_Fov / 2.0f);

	m_State.m_Fulstrum[FRONT].set(Vector3(0, 0, 1), m_State.m_Front);
	m_State.m_Fulstrum[BACK].set(Vector3(0, 0, -1), m_State.m_Back);
	m_State.m_Fulstrum[RIGHT].set(Vector3(-fF, 0.0f, fW).normalize(), 0.0f);
	m_State.m_Fulstrum[LEFT].set(Vector3(fF, 0.0f, fW).normalize(), 0.0f);
	m_State.m_Fulstrum[TOP].set(Vector3(0.0f, -fF, fH).normalize(), 0.0f);
	m_State.m_Fulstrum[BOTTOM].set(Vector3(0.0f, fF, fH).normalize(), 0.0f);
	...
	m_State.m_D = float(m_State.m_Window.width() >> 1) / tan(m_State.m_Fov * 0.5f);
