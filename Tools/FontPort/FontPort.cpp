// FontPort.cpp : Defines the initialization routines for the DLL.
//
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		//TRACE0("FONTPORT.DLL Initializing!\n");
		...
		Port::registerPort( CLASS_KEY( FontPort ), Icon );
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		//TRACE0("FONTPORT.DLL Terminating!\n");

		// Terminate the library before destructors are called
		AfxTermExtensionModule(g_PortDLL);
		// unregister with resourcer
		Port::unregisterPort( CLASS_KEY( FontPort ) );
