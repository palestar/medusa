...
public:
	...
	virtual Box2 XFormScreenToBitmap(const Box2 &rect) const = 0;
	virtual Box2 XFormBitmapToScreen(const Box2 &rect) const = 0;
	...
	virtual void GetDrawableRect(Box2& drawableRect) = 0;

	// refreshes the region of the window, or the entire window if region is NULL
	virtual void RefreshWindow(Box2* pRefreshRegion = NULL) = 0;
