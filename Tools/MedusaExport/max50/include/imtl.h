...
class IMtlParams : public InterfaceServer {
	...
	// Adds rollup pages to the MaxMaterial Params. Returns the window
...
// MaxMaterial and Texmap flags values
...
// MaxMaterial Requirements flags: returned by Requirements() function
...
#define MTLREQ_DISPLACEMAP 		(1<<20) //MaxMaterial has Displacement map channel
#define MTLREQ_SUPERSAMPLE 		(1<<21) //MaxMaterial requires supersampling
...
class MtlBase: public ReferenceTarget, public ISubMap {
	...
	public:
		...
		virtual void SetupGfxMultiMaps(TimeValue t, MaxMaterial *mtl, MtlMakerCallback &cb) {}
...
//		virtual int SubMtlOn(int i) { return 1; } 
		...
		// allows plugins to control the display of the Discard/Keep Old MaxMaterial dialog when being
		// created in a MtlEditor slot.  Added by JBW for scripted material plugins.
		// return TRUE to prevent the Keep Old MaxMaterial dialog from poping.
