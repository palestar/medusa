...
public:
    ...
    virtual void	setMaterial(const MaxMaterial &m, int index=0) = 0;
	virtual MaxMaterial *getMaterial(void) = 0;
	...
	virtual void	loadMeshData(DWORD_PTR id, int xyzCt, Point3 *xyz, int norCt, Point3 *nor, int texNum, int uvwCt, Point3 *uvw, int mtlCt, MaxMaterial *mtl) = 0;
	virtual void	processStrips(DWORD_PTR id, int stripCt, StripTab *s, GFX_ESCAPE_FN fn) = 0;
	virtual void	processWireFaces(int xyzCt, Point3 *xyz, int faceCt, GWFace *face, int dispFlags, BitArray *faceSel, BitArray *edgeSel, int mtlCt, MaxMaterial *mtl, GFX_ESCAPE_FN fn) = 0;
...
#define COLOR_SV_MATERIAL_BK       102 // MaxMaterial Node Background
