...
void DisplayDeviceD3D::updateProjection()
{
	...
	{
		float fAspect = (float)m_Proj.m_rWindow.width() / (float)m_Proj.m_rWindow.height();
		float fh = m_Proj.m_fFOV; 
		float fv = m_Proj.m_fFOV * (1.0f / fAspect);
		float w = 1.0f / tan(fh * 0.5f);
		float h = 1.0f / tan(fv * 0.5f);
